...
Clear();
  ...
  if(not iEvent.getByLabel(patElecLabel_,patElecHandle)){
    std::cout<<"ELE FATAL EXCEPTION: "<<"Following Not Found: "
  ...
  for(ele=eleColl.begin(); ele!=eleColl.end(); ele++){
    ...
    patElecDxy_.push_back(ele->gsfTrack()->dxy());
    patElecDsz_.push_back(ele->gsfTrack()->dsz());
    patElecDz_.push_back(ele->gsfTrack()->dz());
    patElecD0_.push_back(ele->gsfTrack()->d0());
